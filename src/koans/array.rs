...
fn array_map() {
    ...
    let mut iterator = arr.iter().map(|&x| Some(x));
    assert!(iterator.next().unwrap() == Some(arr[0]));
    assert!(iterator.next().unwrap() == Some(arr[1]));
    assert!(iterator.next().unwrap() == Some(arr[2]));
    assert!(iterator.next().unwrap() == Some(arr[3]));
}
...
fn array_filter_map() {
    ...
    let mut iterator = arr.iter().filter_map(|&x| if x == 1 { Some(x + 2) } else { None });
